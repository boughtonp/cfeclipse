...
public class ExternalBrowserAction implements IWorkbenchWindowActionDelegate, IEditorActionDelegate {
	...
	public void run(IAction action) {
		try {
			...
			if (input instanceof FileEditorInput) {
				...
				// Need to figure out what the url is for this file (do the rest
				// if it isnt defined)

				// Try and get the workbench preferneces and project properties
				// for the url!
				String browserAppPath = "";
				String primaryBrowserPath = CFMLPlugin.getDefault().getPluginPreferences().getString(
						BrowserPreferenceConstants.P_PRIMARY_BROWSER_PATH);
				String secondaryBrowserPath = CFMLPlugin.getDefault().getPluginPreferences().getString(
						BrowserPreferenceConstants.P_SECONDARY_BROWSER_PATH);

				browserAppPath = primaryBrowserPath;
				if (action.getActionDefinitionId()
						.equals("org.cfeclipse.cfml.editors.actions.ExternalBrowserActionID2")) {
				...
				if (URLpath.length() == 0) {
					...
					msg.setText("Oops!");
					msg
							.setMessage("The URL for your project has not been defined, edit the CFEclipse properties for this project and enter the URL for this file");
					int result = msg.open();
					/*
					 * if (result == SWT.YES) { IWorkbench wb =
					 * PlatformUI.getWorkbench(); ISelection sel =
					 * (ISelection)fileInput.getFile().getProject();
					 * 
					 * PropertyPage page = new ProjectPropertyPage();
					 * PreferenceManager mgr = new PreferenceManager();
					 * IPreferenceNode node = new PreferenceNode("1", page);
					 * mgr.addToRoot(node); PropertyDialog dialog = new
					 * PropertyDialog(this.editor.getSite().getShell(), mgr,
					 * sel); dialog.create();
					 * dialog.setMessage(page.getTitle()); dialog.open(); }
					 */
					return;

				}
				openUrl(URLpath, browserAppPath);
			 ...
			 * 
			 * FileInputStream in; try { in = new
			 * FileInputStream(fileInput.getFile
			 * ().getProject().getLocation().toFile()+"/cfeclipse.properties");
			 * } catch (FileNotFoundException e) { MessageBox msg = new
			 * MessageBox
			 * (Display.getCurrent().getActiveShell(),SWT.ICON_INFORMATION |
			 * SWT.YES | SWT.NO); msg.setText("Oops!");msg.setMessage(
			 * "There is no cfeclipse.properties file in the root of this project.\nWould you like to have one automatically created now?"
			 * ); int result = msg.open(); if (result == SWT.YES) {
			 * FileOutputStream out = new
			 * FileOutputStream(fileInput.getFile().getProject
			 * ().getLocation().toFile()+"/cfeclipse.properties");
			 * out.write(getDefaultProperties
			 * (fileInput.getFile().getProject()).getBytes()); out.close();
			 * fileInput
			 * .getFile().getProject().refreshLocal(IProject.DEPTH_INFINITE
			 * ,null);
			 * IDE.openEditor(PlatformUI.getWorkbench().getActiveWorkbenchWindow
			 * ().getActivePage(),fileInput.getFile().getProject().getFile(
			 * "cfeclipse.properties")); } return; } Properties p = new
			 * Properties(); p.load(in); if (p.getProperty("BrowserPath") !=
			 * null) { externalBrowser = p.getProperty("BrowserPath"); } if
			 * (p.getProperty("RootURL") != null) { rootURL =
			 * p.getProperty("RootURL"); } if (p.getProperty("URLMask") != null)
			 ...
			 * "The current file is not beneath the webroot for the project. You may need to edit cfeclipse.properties in the root of the project and change the value of the URLMask property."
			 * );
			 * 
			 * return; } filePath = filePath.substring(URLMask.length());
			 * 
			 * } String[] cmd = new String[] { externalBrowser ,rootURL+filePath
			 * };
			 * 
	...
	public void openUrl(String url, String browserAppPath) {
		// Check that the application exists
		File BrowserApp = new File(browserAppPath);
		if (!BrowserApp.exists()) {
			MessageBox msg = new MessageBox(Display.getCurrent().getActiveShell(), SWT.ICON_INFORMATION | SWT.YES
					| SWT.NO);
			msg.setText("Oops!");
		...
		try {
			if (System.getProperty("os.name").equals("Mac OS X")) {
				...
				Runtime.getRuntime().exec(new String[] { "open", "-a", browserAppPath, url });
	...
	private String getDefaultProperties(IProject project) {
		...
		properties
				.append("#The path in the current project to the webroot directory. Including the project name itself.\n");
